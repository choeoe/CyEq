...
public class Cypher2UExpr {
    ...
    public UTerm parseExpr2UTerm(Expression expr) {
        if (expr instanceof Property property) {
            return parseProp2UTerm(property);
            ...
            return UVarTerm.mk(UVar.mkBase(UName.mk(var.name)));
            ...
            if(OUTER)
                CASE = true;
            for (Pair<Expression, Expression> alter : caseExpression.alternatives) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                if (items.get(i).expression instanceof CaseExpression caseExpression) {
                    ...
                    if (caseExpression.alternatives.size() == 1) {
                        ...
                        for (Pair<Expression, Expression> exprs : caseExpression.alternatives) {
